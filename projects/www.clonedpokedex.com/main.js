...
const pokemonLoadConfig = {
  ...
  loadMoreCount: 12  // Number of Pokémon to load when using LoadMorePokemon
}
...
function displayPokemon(pokemon) {
  ...
  // console.log(abilities.length>1? "more than one":"one" )
  // // console.log(ability for (ability in abilities))

  let abilitiesDisplay

  if (abilities.length > 1 && abilities[0].ability.name === abilities[1].ability.name) { 
    console.log("single ability")
    ...
    abilitiesDisplay = abilities.map(ability => 
      `<a href="https://pokemondb.net/ability/${ability.ability.name.toLowerCase()}">${capitalize(ability.ability.name)}</a>`
    ).join(", ");    
  }
