function capitalize(word) {
  ...
  async function searchPokemon() {
    try {
      ...
      displayPokemon(pokemon); // Display the searched Pokémon directly
    } catch (error) {
      alert(error.message);
    }
  }
  ...
  // Fetch Pokemon data from PokeAPI using a structured query
  ...
  async function loadPokedex(limit = 20, offset = 0) {
    try {
  ...
  loadMoreButton.addEventListener("click", async () => {
    currentOffset += 20;
    await loadPokedex(20, currentOffset); 
  });
